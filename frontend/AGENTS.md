...
- **Backend Integration**: Communicates with a Spring Boot / Spring AI backend service:
  ...
  - **REST Persistence API**: Posts saved itineraries (`POST /api/v1/itineraries`) via `saveItineraryToDatabase` in [itineraryApi.ts](file:///home/whyal/Projects/itinerary-planner-fe/app/services/itineraryApi.ts).
...
- `.env.local` sets `NEXT_PUBLIC_API_URL` (default: `http://localhost:8080/api/v1`).
- Endpoints:
